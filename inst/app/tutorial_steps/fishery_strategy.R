intro_steps <- list(
  ...
  list(
    ...
    intro   = "Using this tab will plot both fishing strategies in comparison with eachother."
  ),
  ...
  # Custom: not in previous lists
  list(element = "#fishery_sliders", title = "Fishery Sliders",
       intro = "The sliders here change the fishing effort of each fleet Each
             attempt to model fishing fleets with common characteristics. The fishing strategies
             seen here are considered the current fishing strategies."
  ),
  # Copied: species plot description from biomass tutorial
...
from 3 times plotted to only the chosen time plotted."
  ...
  # Copied: same as biomass size spectrum explanation
  ...
  # Copied: guild plot from biomass tutorial
  ...
  list(element = "#fishy_plots .nav-link[data-value='Spectra']", title = "Size Spectrum Plot",
       intro = "This plot is the size spectrum (the biomass distribution across sizes of fish) of each
             species of fish in the model. If comparing simulations, sim 2 will be a dashed line."
  ),
